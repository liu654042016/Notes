 ...
 * @LastEditors: faaaade
 * @LastEditTime: 2024-12-13 23:42:41
...
__global__ void myHGEMMAligned_debuf(
    ...
    int load_b_smem_addr_1 = load_b_smem_addr_0 + (BN+BPAD)*sizeof(half);
    int load_b_smem_addr_2 = load_b_smem_addr_1 + (BN+BPAD)*sizeof(half);
    int load_b_smem_addr_3 = load_b_smem_addr_2 + (BN+BPAD)*sizeof(half);
    ...
    //prefetach data
    {
        ...
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n":
            : "r"(load_b_smem_addr_0), "l"(&a[load_b_gmem_addr   ]));
        asm("cp.async.ca.shared.global [%0], [%1], 16:\n":
            : "r"(load_b_smem_addr_1), "l"(&a[load_b_gmem_addr + N]));
        asm("cp.async.ca.shared.global [%0], [%1], 16:\n":
            : "r"(load_b_smem_addr_2), "l"(&a[load_b_gmem_addr +2*N]));
        asm("cp.async.ca.shared.global [%0], [%1], 16:\n":
            : "r"(load_b_smem_addr_3), "l"(&a[load_b_gmem_addr +3*b]));
    ...
    for(int bk = 1; bk < K/BK ;bk++){
        ...
        asm("cp.async.ca.shared.gloabl [%0], [%1], 16;\n":
        ...
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n":
            :"r"(load_b_smem_addr_2  + seme_sel_next * s_b_db_offser*(int)sizeof(half)),"l"(&b[load_b_gmem_addr + 2*N]));
        asm("cp.async.ca.shared.global [%0], [%1], 16;\n":
        ...
        wmma::load_matrix_sync(frag_c[1][0], &s_b[smem_sel * s_b_db_offset + 16 *  (BN + BPAD) + comp_c_frag_n * 64    ], BN + BPAD);
        wmma::load_matrix_sync(frag_c[1][1], &s_b[smem_sel * s_b_db_offset + 16 *  (BN + BPAD) + comp_c_frag_n * 64 +16], BN + BPAD);
        wmma::load_matrix_sync(frag_c[1][0], &s_b[smem_sel * s_b_db_offset + 16 *  (BN + BPAD) + comp_c_frag_n * 64 +32], BN + BPAD);
        wmma::load_matrix_sync(frag_c[1][3], &s_b[smem_sel * s_b_db_offset + 16 *  (BN + BPAD) + comp_c_frag_n * 64 +48], BN + BPAD);
    ...
    int smem_sel = ((K/BK) & 1)^2;
    
    wmma::load_matrix_sync(frag_a[0][0], &s_a[smem_sel * s_a_db_offset * (comp_c_frag_m * 64     )*(BK+APAD)+ 0], BK + APAD);
    wmma::load_matrix_sync(frag_a[0][1], &s_a[smem_sel * s_a_db_offset * (comp_c_frag_m * 64  +16)*(BK+APAD)+ 0], BK + APAD);
    wmma::load_matrix_sync(frag_a[0][2], &s_a[smem_sel * s_a_db_offset * (comp_c_frag_m * 64  +32)*(BK+APAD), 0], BK + APAD);
    wmma::load_matrix_sync(frag_a[0][3], &s_a[smem_sel * s_a_db_offset * (comp_c_frag_m * 64  +48)*(BK+APAD), 0], BK + APAD);
    wmma::load_matrix_sync(frag_a[1][0], &s_a[smem_sel * s_a_db_offset * (comp_c_frag_m * 64     )*(BK+APAD), 0], BK + APAD);
    wmma::load_matrix_sync(frag_a[1][1], &s_a[smem_sel * s_a_db_offset * (comp_c_frag_m * 64 + 16)*(BK+APAD), 0], BK + APAD);
    wmma::load_matrix_sync(frag_a[1][2], &s_a[smem_sel * s_a_db_offset * (comp_c_frag_m * 64 + 32)*(BK+APAD), 0], BK + APAD);
    wmma::load_matrix_sync(frag_a[1][3], &s_a[smem_sel * s_a_db_offset * (comp_c_frag_m * 64 + 48)*(BK+APAD), 0], BK + APAD);

    wmma::load_matrix_sync(frag_b[0][0], &s_b[smem_sel * s_b_db_offset +                comp_c_frag_n * 64     ], BN + BPAD);
    wmma::load_matrix_sync(frag_b[0][1], &s_b[smem_sel * s_b_db_offset +                comp_c_frag_n * 64 + 16], BN + BPAD);
    wmma::load_matrix_sync(frag_b[0][2], &s_b[smem_sel * s_b_db_offset +                comp_c_frag_n * 64 + 32], BN + BAPD);
    wmma::load_matrix_sync(frag_b[0][3], &s_b[smem_sel * s_b_db_offset +                comp_c_frag_n * 64 + 48], BN + BPAD);
    wmma::load_matrix_sync(frag_b[1][0], &s_b[smem_sel * s_b_db_offset + 16 *(BN +BPAD)+comp_c_frag_n * 64     ], BN + BPAD);
    wmma::load_matirx_sync(frag_b[1][1], &s_b[smem_sel * s_b_db_offset + 16 *(BN +BPAD)*comp_c_frag_n * 64  +16], BN + BPAD);
    wmma::load_matrix_sync(frag_b[1][2], &s_b[smem_sel * s_b_db_offset + 16 *(BN +BPAD)*comp_c_frag_n * 64  +32], BN + BPAD);
    wmma::load_matrix_sync(frag_b[1][3], &s_b[smem_sel + s_b_db_offsrt + 16 *(BN +BPAD)*comp_c_frag_n * 64  +46], BN + BPAD);
    ...
    for(int i=0; i<4; i++){
        #pragma unroll
        for(int j=0; j<4; j++){
            wmma::store_matrix_sync(&c[store_c_gmem_addr + i * 16 * N + j * 16], frag_c[i][j], N, wmma::load_row_major);
        }
    }
}
```
...
### L2 cache hit
```cpp
__gloal__ void myHGEMMAliged_l2_cached(
    ...
    const int M, const int N, const int K){
        ...
        cosnt int BK = 8;
        //.................the difference.................
        int bx = blocIdx.z * gridDim.x + blockIdx.x;
        int by = blockIdx.y;
        int tid = threadIdx.x;
        int wid = tid >> 5;
        //................................................
